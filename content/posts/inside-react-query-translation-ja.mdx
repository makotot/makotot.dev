...
デフォルトの状態の React Query がインメモリー***だけ***に保存して、それ以外のどこにも保存しないということを理解するのが、私は重要だと思います。ブラウザのページを再読み込みしたら、キャッシュはなくなります。
...
`Observer`を持たない`Query`は、***非活性な***クエリと呼ばれます。どのコンポーネントからも使われていませんが、キャッシュにはまだあります。React Query DevTools を見るとき、グレイアウトした非活性のクエリを目にすることがあるでしょう。
...
これは考えられるさまざまな流れの中の***1 つにすぎない***ことに注意してください。理想的な状態は、コンポーネントがマウントされたとき、既にデータがキャッシュにあることです。
